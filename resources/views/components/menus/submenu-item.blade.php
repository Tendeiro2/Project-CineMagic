@props(['content', 'selectable' => true, 'selected' => false, 'href' => '#'])

@if($selectable)
    @if($selected)
        <a {{ $attributes->merge(['class' => 'px-3 py-4 border-b-2 border-b-indigo-400 dark:border-b-indigo-500
    ...
    @else
        <a {{ $attributes->merge(['class' => 'px-3 py-4 border-b-2 border-transparent
                text-sm font-medium leading-5 inline-flex h-auto
